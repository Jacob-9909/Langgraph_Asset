...
export default function DashboardPage() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500 dark:text-gray-400">로딩 중...</div>
  if (!data) return <div className="p-8 text-center text-red-500 dark:text-red-400">데이터를 불러올 수 없습니다.</div>
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 shadow-sm dark:shadow-none">
          <div className="text-gray-600 dark:text-gray-400 text-sm mb-1">총 자산</div>
          <div className="text-2xl font-bold text-emerald-400">{formatKRW(data.total_assets_krw)}원</div>
        </div>
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 shadow-sm dark:shadow-none">
          <div className="text-gray-600 dark:text-gray-400 text-sm mb-1">보유 자산 수</div>
          <div className="text-2xl font-bold">{data.asset_count}건</div>
        </div>
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 shadow-sm dark:shadow-none">
          <div className="text-gray-600 dark:text-gray-400 text-sm mb-1">최근 상담</div>
      ...
      {/* Assets by Type */}
      {data.asset_count > 0 && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 mb-8 shadow-sm dark:shadow-none">
          <h2 className="text-lg font-semibold mb-4">자산 유형별 분포</h2>
          <div className="space-y-2">
            {Object.entries(data.assets_by_type).map(([type, amount]) => (
              <div key={type} className="flex justify-between text-sm">
                <span className="text-gray-700 dark:text-gray-300">{ASSET_TYPE_LABELS[type] || type}</span>
                <span className="text-gray-600 dark:text-gray-400 tabular-nums">{formatKRW(amount)}원</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gray-600 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="text-left py-2 font-medium">자산명</th>
                  <th className="text-right py-2 font-medium">금액</th>
                </tr>
              </thead>
              <tbody>
                {data.assets.map((a) => (
                  <tr key={a.id} className="border-b border-gray-100 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                    <td className="py-2 text-gray-600 dark:text-gray-400">{ASSET_TYPE_LABELS[a.asset_type] || a.asset_type}</td>
